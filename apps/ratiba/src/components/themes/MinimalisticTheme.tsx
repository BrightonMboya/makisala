      ...
      <Map bounds={bounds} minZoom={4} maxZoom={12}>
        ...
        {locations.map((loc, idx) => (
          <MapMarker key={loc.name} longitude={loc.coordinates[0]} latitude={loc.coordinates[1]}>
            <MarkerContent>
              <motion.div
                className="flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-stone-800 px-1 text-[9px] font-bold whitespace-nowrap text-white shadow-md"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{
                  type: 'spring',
...
export default function MinimalisticTheme({ data, onHeroImageChange, onDayImageChange }: MinimalisticThemeProps) {
  ...
  const bookingHref = `/proposal/${data.id}/book`;
