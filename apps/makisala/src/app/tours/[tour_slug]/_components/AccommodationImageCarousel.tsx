...
export default function AccommodationImageCarousel({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            >
                <DialogTitle className="sr-only">{accommodationName} Gallery</DialogTitle>
                    ...
                    {images.length > 1 && (
                        <>
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute top-1/2 left-2 -translate-y-1/2 rounded-full bg-black/50 text-white hover:bg-black/70 md:-left-12"
                                onClick={e => {
                                    e.stopPropagation()
                                    prevImage()
                                }}
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute top-1/2 right-2 -translate-y-1/2 rounded-full bg-black/50 text-white hover:bg-black/70 md:-right-12"
                                onClick={e => {
                                    e.stopPropagation()
                                    nextImage()
                                }}
