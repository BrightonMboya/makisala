...
const countryOgImages: Record<string, string> = {
    tanzania:
        'https://res.cloudinary.com/dr2tdyz2w/image/upload/v1763039383/Serengeti-Wildebeest-Migration-Safari-10-Days-750x450_lflnpq.jpg',
...
export async function generateMetadata({ params }: Params) {
    ...
    return {
        ...
        openGraph: {
            title: `${capitalize(country)} Safaris | Makisala Safaris`,
            description: `Discover the best safaris in ${capitalize(country)} offered by Makisala Safaris`,
            images: countryOgImages[country]
                ? [
                      {
                          url: countryOgImages[country],
                          width: 1200,
                          height: 630,
                          alt: `${capitalize(country)} Safaris`,
                      },
                  ]
                : undefined,
        },
...
export default async function Page({ params }: Params) {
    const { country } = await params
    const [tours] = await Promise.all([getToursByCountry(country)])
                    ...
                    __html: JSON.stringify([
                        ...
                        TouristDestinationSchema({
                            ...
                            description:
                                ...
                                `Discover the best safaris in ${capitalize(country)} offered by Makisala Safaris`,
