    ...
    <div className="mx-auto max-w-4xl px-8 text-center lg:px-16">
      ...
      >
        ...
        <p className="mx-auto max-w-2xl text-lg leading-relaxed font-light text-stone-500 lg:text-xl">
          An intimate exploration of nature's grandeur, where every sunrise brings new wonders and
          every evening ends with stories worth telling.
        </p>
      </motion.div>
    </div>
  </section>
      ...
      {hasActivities && (
        <div className="bg-white py-16 lg:py-24">
          <div className="mx-auto max-w-5xl px-8 lg:px-16">
              ...
              {day.activities.map((activity, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="group relative rounded-2xl bg-stone-50 p-6 transition-colors hover:bg-stone-100 lg:p-8"
                  >
                    <div className="flex items-start gap-6">
                      {/* Icon */}
                      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm transition-shadow group-hover:shadow-md">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mb-3 flex items-center gap-3">
                          <span className="inline-flex items-center gap-2 text-sm text-stone-500">
                            <TimeIcon time={activity.time} className="h-4 w-4" />
                            {timeOfDay}
                          </span>
                          {activity.location && (
                            <>
                        ...
                        {/* Activity name */}
                        <h4 className="mb-3 font-serif text-xl text-stone-800 lg:text-2xl">
                          {activity.activity}
                        </h4>

                        {/* Description */}
                        {activity.description && (
                          <p className="text-base leading-relaxed font-light text-stone-500">
                            {activity.description}
                          </p>
                        )}
                      </div>
            ...
            {/* Day Overview - The narrative for this day */}
            {day.description && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                <p className="mb-6 text-xs font-light tracking-[0.3em] text-stone-400 uppercase">
                  Day Overview
                </p>
                <p className="text-lg leading-relaxed font-light text-stone-600 lg:text-xl">
                  {day.description}
                </p>
              </motion.div>
            )}
              ...
              <div className="max-w-lg">
                ...
                <div className="flex flex-wrap gap-4">
                  <div className="rounded-xl border border-stone-200 bg-white px-5 py-4">
                    <p className="mb-1 text-xs tracking-wider text-stone-400 uppercase">Stay</p>
                    <p className="font-medium text-stone-800">1 Night</p>
                  </div>

                  {mealsArray.length > 0 && (
                    <div className="rounded-xl border border-stone-200 bg-white px-5 py-4">
                      <p className="mb-1 text-xs tracking-wider text-stone-400 uppercase">Meals</p>
                      <div className="flex items-center gap-2">
                        {mealsArray.map((meal, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center gap-1.5 text-sm font-medium text-stone-700"
                          >
                            {meal === 'Breakfast' && <Coffee className="h-4 w-4 text-amber-600" />}
                            {meal === 'Lunch' && (
                              <UtensilsCrossed className="h-4 w-4 text-orange-600" />
                            )}
                            {meal === 'Dinner' && <Wine className="h-4 w-4 text-rose-600" />}
                            {meal}
                          </span>
                        ))}
                      </div>
  ...
  return (
    <div className="w-full bg-white">
      ...
      {data.itinerary.map((day, index) => (
        <DaySection key={day.day} day={day} data={data} isEven={index % 2 === 1} />
      ))}
