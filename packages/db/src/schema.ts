...
export const account = pgTable(
  'account',
  {
    ...
    createdAt: timestamp('created_at').notNull(),
    updatedAt: timestamp('updated_at').notNull(),
    // Better Auth 1.7+: account identity is scoped by issuer, not just provider.
...
// ---------- OAUTH PROVIDER (Better Auth MCP/OAuth Plugin) ----------
...
export const proposals = pgTable('proposals', {
  ...
  // Snapshotted: the operator can edit the proposal afterwards, so recomputing
  // later would not reproduce what the client agreed to.
...
// Ids reference proposal_activities.id, proposalDays.alternatives[].id (keyed
// by day id), and proposals.extras[].id.
...
export interface AlternativeAccommodation {
  ...
  // vehicle"). For every other basis the label is derived from priceBasis so
  // it can never disagree with what's actually charged.
  priceUnitLabel?: string | null;
  // How `additionalPrice` scales. An alternative covers one night, so "per
  // night" is the flat case; `rooms` gives the count for `per_room`; `custom`
  // bills once, like `flat`, but shows `priceUnitLabel` instead. Written on
  // create so a missing value means a pre-0061 row, not a deliberate flat.
...
export const proposalActivities = pgTable('proposal_activities', {
  ...
  proposalDayId: uuid('proposal_day_id')
    .notNull()
    .references(() => proposalDays.id, { onDelete: 'cascade' }),
  activityLibraryId: uuid('activity_library_id').references(() => activityLibrary.id, {
    onDelete: 'set null',
  }),
  name: text('name').notNull(),
  description: text('description'),
  ...
  // Optional activities only: the pricing engine excludes them from the quote,
  // so this is the only place their price lives. Null = offered "on request".
...
export const activityLibraryRelations = relations(activityLibrary, ({ one, many }) => ({
  organization: one(organizations, {
...
export type ActivityRate = typeof activityRates.$inferSelect;
export type NewActivityRate = typeof activityRates.$inferInsert;
...
export const activityRates = pgTable(
  'activity_rates',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    organizationId: uuid('organization_id')
    ...
    activityId: uuid('activity_id')
      .notNull()
      .references(() => activityLibrary.id, { onDelete: 'cascade' }),
    seasonId: uuid('season_id').references(() => seasons.id, { onDelete: 'cascade' }),
    chargeBasis: ActivityChargeBasis('charge_basis').notNull().default('per_person'),
    rate: numeric('rate', { precision: 12, scale: 2 }).notNull(),
    currency: text('currency').notNull().default('USD'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
  ...
  (table) => [
    index('idx_activity_rates_org').on(table.organizationId),
    index('idx_activity_rates_activity').on(table.activityId),
  ],
);

export const activityRatesRelations = relations(activityRates, ({ one }) => ({
  organization: one(organizations, {
    fields: [activityRates.organizationId],
    references: [organizations.id],
  }),
  activity: one(activityLibrary, {
    fields: [activityRates.activityId],
    references: [activityLibrary.id],
  }),
  season: one(seasons, {
    fields: [activityRates.seasonId],
    references: [seasons.id],
  }),
}));
