...
type AnimatedRouteMarkerProps = {
  /** Array of [longitude, latitude] coordinate pairs defining the route path */
  coordinates: [number, number][];
  /** Duration of one complete animation cycle in milliseconds (default: 10000) */
  duration?: number;
  /** Color of the animated marker (default: "#22c55e") */
  color?: string;
  /** Size of the marker in pixels (default: 12) */
  size?: number;
  /** Whether to show the pulsing glow effect (default: true) */
  showGlow?: boolean;
  /** Whether to loop the animation (default: true) */
  loop?: boolean;
};

function AnimatedRouteMarker({
  coordinates,
  duration = 10000,
  color = "#22c55e",
  size = 12,
  ...
  // Calculate total route distance for interpolation
  ...
  const getPositionAtProgress = useCallback(
    (progress: number): [number, number] => {
      ...
      const targetDistance = progress * routeData.totalLength;
      let accumulatedDistance = 0;

      for (const segment of routeData.segments) {
        if (accumulatedDistance + segment.length >= targetDistance) {
          const segmentProgress =
            (targetDistance - accumulatedDistance) / segment.length;
          return [
  ...
  useEffect(() => {
    ...
    el.className = "animated-route-marker";
    el.innerHTML = `
      <div style="
        width: ${size}px;
        height: ${size}px;
        ...
        box-shadow: 0 0 ${showGlow ? "12px" : "4px"} ${color}${showGlow ? ", 0 0 24px " + color + "80" : ""};
        animation: ${showGlow ? "pulse 1.5s ease-in-out infinite" : "none"};
      "></div>
      <style>
        @keyframes pulse {
  ...
  }, [isLoaded, map, coordinates, duration, color, size, showGlow, loop, getPositionAtProgress]);
