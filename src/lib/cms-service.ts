...
export const cmsService = {
  ...
  async getPublishedPages(pageType?: "page" | "blog"): Promise<PageData[]> {
    let query = "SELECT * FROM pages WHERE status = ? ORDER BY created_at DESC";
    let params: any[] = ["published"];

    if (pageType) {
      query =
        "SELECT * FROM pages WHERE status = ? AND page_type = ? ORDER BY created_at DESC";
      params = ["published", pageType];
    }

    const stmt = db.prepare(query);
    return stmt.all(...params) as PageData[];
  },
};
